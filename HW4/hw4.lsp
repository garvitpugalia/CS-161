...
(defun node2var (n c k)
  (+ (* (- n 1) k) c) ;using the equation (c + ((n - 1) * k))
  )
...
(defun at-least-one-color (n c k)
  (cond ((<= c k) (cons (node2var n c k) (at-least-one-color n (+ c 1) k))) ;check each color from {c, c+1, ..., k} and create a list of clauses 
	(t '())) ;if C > K, all colors have been utilized, return empty
  )
...
;
(defun create-color-list (n c cc k)
  (list (- (node2var n c k)) (- (node2var n cc k)))
  ;returns a list of negated clauses (same node, multiple colors)
)
  
(defun get-pairs-of-colors (n c k cc)
  (cond ((> c k) '()) ;if C > K, all colors have been utilized, return empty
	((> cc k) (get-pairs-of-colors n (+ c 1) k (+ c 2))) ;look at each pair of colors (c, c+1)
	(t (cons (create-color-list n c cc k) (get-pairs-of-colors n c k (+ cc 1)))))
	;node n doesn't have colors c or cc, keep looking for other colors
  )

(defun at-most-one-color (n c k)
  (get-pairs-of-colors n c k (+ c 1)) ;look for all pairs from {c, c+1, ... ,k}
  )
...
(defun generate-node-clauses (n k)
  ...
  ;one color = at most one color (and) at least one color from {1,..., k}
  )
...
;
(defun create-node-list (n1 n2 c k)
  (list (- (node2var n1 c k)) (- (node2var n2 c k)))
  ;returns a list of negated clauses (different nodes, same color)
  )

(defun count-top-of-list (top top-rest i k)
  (cond ((<= i k) (cons (create-node-list top top-rest i k) (count-top-of-list top top-rest (+ i 1) k)))
	;connected nodes at (car e) and (cadr e) are not allowed to be same color
	(t '())) ;if I > K, all colors have been exhausted
  )
  
(defun generate-edge-clauses (e k)
  (count-top-of-list (car e) (cadr e) 1 k) ;run through all connected nodes, and all colors {1, ..., k}
  )
